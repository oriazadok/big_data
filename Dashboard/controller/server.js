...
app.get('/', async (req, res) => {
  ...
  try {
    const dataFromSimbad = await simbad(last_event.RA, last_event.DEC || last_event.dec);
    ...
    if (client.readyState === WebSocket.OPEN) {
      getMostRecentDateTime()
        .then((last_event) => {
          ...
          if((! lastEventsIds.includes(last_event.id)) && last_event.urgency == 5) {
            lastEventsIds.push(last_event.id);
