...
async function readElasticSearch(query) { 
  ...
  async function executeQuery(query) {
    ...
    if(query) {
      ...
      for (const key in query) {
        const modifiedKey = key.replace(/'/g, ''); // Remove any quotes around the key
        query[modifiedKey] = query[key];
        // delete query[key]
      }
    }
...
//   const data = {
//     currentDate: '2023-07-18 15:30', // Replace this with the actual date and time
//     json: {
//       rise: { status: 'RISE', 'Azimuth:': '68.9°', time: '5:03' },
//       transit: { status: 'TRANSIT', 'Max altitude:': '59.6°', time: '13:04' },
//       set: { status: 'SET', 'Azimuth:': '291.0°', time: '21:05' }
//     }
//   };
//   res.render("pages/check", data);
// })
